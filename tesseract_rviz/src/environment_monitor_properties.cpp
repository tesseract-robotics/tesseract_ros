...
struct EnvironmentMonitorProperties::Implementation
{
  ...
  rviz::RosTopicProperty* environment_snapshot_topic_property{ nullptr };
  ...
  : data_(std::make_unique<Implementation>())
{
  ...
  data_->environment_snapshot_topic_property =
      new rviz::RosTopicProperty("Snapshot Topic",
                                 "/tesseract_environment_snapshot",
                                 ros::message_traits::datatype<tesseract_msgs::Environment>(),
...
void EnvironmentMonitorProperties::snapshotCallback(const tesseract_msgs::Environment::ConstPtr& msg)
{
  if (data_->scene_manager == nullptr || data_->scene_node == nullptr)
    return;
  ...
  {
    data_->parent->setStatus(rviz::StatusProperty::Error, "Tesseract", "Snapshot failed to load from message");
  }
}
...
void EnvironmentMonitorProperties::onEnvironmentSnapshotTopicChanged()
{
  ...
  data_->snapshot = data_->nh.subscribe(data_->environment_snapshot_topic_property->getTopicStd(),
                                        10,
                                        &EnvironmentMonitorProperties::snapshotCallback,
                                        this);
}
